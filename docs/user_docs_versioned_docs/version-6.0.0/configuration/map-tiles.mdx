...
After defining your map tiles, add them to the `connect-src` directive in
`TALISMAN_CONFIG` and `TALISMAN_CONFIG_DEV` so the browser's Content Security
Policy allows the frontend to fetch tiles from those origins:
...
:::note
`ENABLE_CORS` is **not** required for map tiles. CORS controls which external
origins may make cross-origin requests *to* the Superset API, whereas map tiles
are fetched *from* external tile servers by the browser. The CSP `connect-src`
directive above is what permits those outbound fetches.
:::
